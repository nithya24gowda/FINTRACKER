...
<h2>PROJECT_ALLOC DETAILS</h2>

<?php include('serverproject_costing.php');
...
if(isset($_GET['edit'])) {
  ...
  $edit_state=true;
  $rec = mysqli_query($db,"SELECT * FROM project_costing WHERE project_id=$project_id");
...
<a href="addproject_costing.php" id ="project_costing" class="btn button_color <?php echo ((basename($_SERVER['PHP_SELF'])=='project_costing.php')?'activeSubMenu':'');?>" >&nbsp;Add New</a>  
<br><br>
<table>
        <thead>
          <tr>
        ...
        <tbody>
          <?php while ($row= mysqli_fetch_array($results)){ ?>
            <tr>
              ...
              <td>
                <a class="edit_btn" href="addproject_costing.php?edit=<?php echo $row['project_id']; ?>">Edit</a>
              </td>
              <td>
                <a class="del_btn" href="serverproject_costing.php?del=<?php echo $row['project_id']; ?>">Delete</a>
              </td>
            </tr>
          <?php }?>
        </tbody>
